...
用 Spotlight 扫描同一 App 的多份磁盘副本,用 `lsregister -u` 精准反注册冗余项——绝不用会损坏系统设置的 `-kill`。
...
全部仅用 macOS 自带 CLI,零外部依赖。在 **设置 › 右键菜单** 查看/编辑,在 **设置 › 通用** 恢复出厂。

| 脚本 | 动作 | 说明 |
|------|------|------|
| `copy-path.sh` | 复制路径 | `pbcopy`,多选每行一个路径 |
| `new-file.sh` | 新建文件 | 子菜单列举模板目录;`cp` + 自动重名编号 |
| `open-terminal.sh` | 在终端打开 | 终端由「通用」决定(`MENUMATE_TERMINAL`) |
| `open-editor.sh` | 用编辑器打开 | 编辑器由「通用」决定(`MENUMATE_EDITOR`) |
| `image-convert.sh` | 图片转换 | 子菜单 png/jpeg/heic/tiff,`sips`;仅图片 |
